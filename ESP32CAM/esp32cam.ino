...
//輸入WIFI連線帳號密碼
const char* ssid = "VIRUS666";
const char* password = "33264672(!@#)";
...
//Exibir o status do parâmetro de vídeo (deve retornar o formato json para carregar as configurações iniciais)
...
<html>
    ...
    <body>
        ...
        function alerta(alerta){
            ...
            fetch(`http://10.0.0.132/${alerta}`, myInit).then(()=>{
                console.log('enviado alerta nodejs com sucesso!!!')
            }).catch((err)=>{
                console.log(err)
            })
        ...
        function CheckStatus(){
            if(STATUS[0].classe && STATUS[0].porcentagem > 0.60){
                alerta('alerta')
                console.log(`${STATUS[0].classe}\n${STATUS[0].porcentagem}`)
            }
        }
        ...
        async function predict() {
          ...
          if (maxPredictions>0) {
            for (let i = 0; i < maxPredictions; i++) {
              ...
              STATUS.pop()
                STATUS.push({classe: prediction[i].className, porcentagem: prediction[i].probability})
            }
              else {
